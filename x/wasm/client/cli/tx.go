...
import (
	...
	"github.com/airchains-network/junction/x/wasm/ioutils"
...
func parseInstantiateArgs(rawCodeID, initMsg string, kr keyring.Keyring, sender string, flags *flag.FlagSet) (*types.MsgInstantiateContract, error) {
	...
	if adminStr == "" && !noAdmin {
		return nil, fmt.Errorf("you must set an admin or explicitly pass --no-admin to make it immutible (junctiond issue #719)")
	}
	if adminStr != "" && noAdmin {
