...
func TestGasCostOnQuery(t *testing.T) {
	const (
		...
		GasReturnUnhashed uint64 = 36
		GasReturnHashed   uint64 = 29
	)
...
func TestLimitRecursiveQueryGas(t *testing.T) {
	...
	const (
		// Note: about 100 SDK gas (10k CosmWasm gas) for each round of sha256
		GasWork2k uint64 = 78_937 // = SetupContractCost + x // we have 6x gas used in cpu than in the instance
		// This is overhead for calling into a sub-contract
		GasReturnHashed uint64 = 30
	)
	...
	cases := map[string]struct {
		...
		"very deep recursion, hits recursion limit": {
			...
			expectError:               "query wasm contract failed",          // Error we get from the contract instance doing the failing query, not junctiond
			expectedGas:               10*(GasWork2k+GasReturnHashed) + 3152, // lots of additional gas for long error message
		},
	}
